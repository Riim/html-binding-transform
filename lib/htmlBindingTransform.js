...
/**
 * @typesign (data: Array<string>, templateDelimiters: Array<string>): { bindingExpr: string, newData: string };
 */
function parseData(data, templateDelimiters) {
	var bindingExpr = [];
	var newData = [];

	data.forEach(function(piece, index) {
		if (index % 2) {
			bindingExpr.push('this.' + piece + '()');
		...
		if (node.type == 'tag') {
			...
			for (var name in attrs) {
				...
				if (data.length > 1) {
					...
					data = parseData(data, templateDelimiters);
